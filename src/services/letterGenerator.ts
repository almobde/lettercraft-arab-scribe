...
${organization}`;
      ...
      // Control length based on letterLength setting
      const targetLength = getTargetLengthForSpecial(data.letterLength);
      if (finalLetter.length < targetLength.min) {
        const additionalContent = `

نسأل الله العظيم رب العرش الكريم أن يديم عليكم نعمه الظاهرة والباطنة، وأن يزيدكم من فضله وكرمه وجوده، وأن يجعل أيامكم كلها خيراً وبركة وسعادة وهناء.`;
  ...
  // المعالجة العادية للمناسبات الأخرى باستخدام OpenAI
  try {
    ...
    let generatedLetter = await generateWithOpenAI(prompt);
...
const generateEnglishTranslation = async (data: LetterData): Promise<string> => {
  ...
  try {
    ...
    const translation = await generateWithOpenAI(prompt);
    return translation;
  } catch (error) {
...
const getTargetLengthForSpecial = (length: 'قصير' | 'متوسط' | 'طويل'): { min: number; max: number } => {
  switch (length) {
    case 'قصير':
...
const generateCreativeVersion = (data: LetterData): string => {
  ...
  // Enhanced creative version with length control
  const targetLength = getTargetLengthForSpecial(data.letterLength);
  ...
  // Select content based on length requirements
  const greeting = creativeGreetings[0]; // Keep it simple for now
  ...
  if (data.occasion) {
    ...
    // Adjust body length based on letterLength
    if (data.letterLength === 'قصير') {
      body = `لقد استطعتم بجدارة أن تحولوا ${enhancedOccasion} إلى واقع مشرق ينير دروب المستقبل. إن عملكم المتقن وتفانيكم الصادق سيبقيان مثالاً يحتذى به للأجيال القادمة.

نسأل الله أن يبارك في مساعيكم ويوفقكم لما فيه الخير والصلاح.`;
    } else if (data.letterLength === 'متوسط') {
      body = `لقد استطعتم بجدارة واقتدار كبير أن تحولوا ${enhancedOccasion} إلى واقع مشرق باهر ينير دروب المستقبل الزاهر. إن عملكم المتقن الرائع وتفانيكم الصادق النبيل سيبقيان مثالاً يحتذى به للأجيال القادمة.

إن ما قدمتموه من جهود متميزة وعمل دؤوب مستمر يستحق كل التقدير والإعجاب والثناء العطر. فقد كان لعملكم المتقن أثر بالغ في تحقيق النجاح الباهر والتميز المنشود.

نسأل الله العظيم أن يبارك في جهودكم المباركة، وأن يوفقكم دائماً لما فيه الخير والصلاح والفلاح.`;
    } else {
      // طويل - use expanded content
      const toneKey = data.tone as keyof typeof expandedMiddleContent;
      const middleContent = expandedMiddleContent[toneKey] ? expandedMiddleContent[toneKey][0] : '';
      
      body = `لقد استطعتم بجدارة واقتدار كبير أن تحولوا ${enhancedOccasion} إلى واقع مشرق باهر ينير دروب المستقبل الزاهر بنوره الساطع. إن عملكم المتقن الرائع وتفانيكم الصادق النبيل سيبقيان مثالاً يحتذى به للأجيال القادمة التي ستتعلم منكم معنى الإخلاص الحقيقي في العمل.

${middleContent}

نسأل الله العلي العظيم القدير أن يبارك في مساعيكم النبيلة ويضاعف أجركم عنده، وأن يجعل عملكم المبارك في ميزان حسناتكم، وأن يوفقكم إلى ما فيه خير الإسلام والمسلمين.`;
    }
  }
